...
func (a *NodeAPI) ListByApplicationID(ctx context.Context, req *pb.ListDeviceByApplicationIDRequest) (*pb.ListDeviceResponse, error) {
	...
	devices, err := storage.GetDevicesForApplicationID(common.DB, req.ApplicationID, int(req.Limit), int(req.Offset))
	if err != nil {
		return nil, errToRPCError(err)
	}
	count, err := storage.GetDeviceCountForApplicationID(common.DB, req.ApplicationID)
	if err != nil {
		return nil, errToRPCError(err)
	}
